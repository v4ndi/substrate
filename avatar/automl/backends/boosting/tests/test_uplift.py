...
def test_x_learner_effect_formula_and_constant_propensity() -> None:
    ...
    effect, control, treated, propensity = backend._predict_learner("x", components, frame, _schema())
...
def test_x_formula_uses_bounded_effect_regressors_before_propensity_combination() -> None:
...
def test_x_learner_pseudo_outcomes_follow_the_standard_definition(monkeypatch) -> None:
    ...
    monkeypatch.setattr(backend, "_fit_t", lambda *args, **kwargs: dict(outcome_components))

    def capture_effect_fit(kind, params, train, train_target, valid, valid_target, schema):
...
def test_hyperopt_keeps_each_already_fitted_best_composite(monkeypatch) -> None:
    ...
    monkeypatch.setattr(backend, "_fit_t", lambda params, *args, **kwargs: fake_fit(params))
...
def test_compatible_constituents_reuse_prepared_catboost_data(monkeypatch) -> None:
    ...
    backend = UpliftBoostingBackend(engine="catboost", params={}, random_state=42, device="cpu")
    backend._reuse_prepared = True
    monkeypatch.setattr(backend, "_component", lambda kind, params: FakeComponent())
    frame = pl.DataFrame({"x": [0.0, 1.0]})
    target = np.array([0, 1])
    for _ in range(2):
        backend._fit_component("classifier", {}, frame, target, frame, target, _schema(), cache_key="shared")
...
def _split(rows: int, seed: int) -> pl.DataFrame:
    ...
    probability = 1 / (1 + np.exp(-(-0.3 + 0.7 * x + treatment * (0.8 + 0.4 * (x > 0)))))
...
def test_real_engine_composite_round_trip(tmp_path, engine: str, model_params: dict) -> None:
    ...
    assert prediction.scores.columns == ["epk_id", "report_month", "treatment", *UPLIFT_SCORE_COLUMNS]
    ...
    arm_evaluation = task.evaluate(paths[2], prediction, metrics=["treatment_roc_auc", "control_roc_auc"])
    assert {"s_qini_auc", "t_uplift_at_20", "x_control_roc_auc"} <= set(evaluation.metrics_raw)
    ...
    assert evaluation.metrics_by_group_raw["learner"].unique().sort().to_list() == ["s", "t", "x"]
    assert {"metrics_raw", "feature_importance_raw"} <= set(evaluation.excel_paths_raw)
    assert set(plt.get_fignums()) == open_figures
    shuffled_evaluation = task.evaluate(paths[2], PredictionResult(prediction.scores.reverse()))
    ...
    with pytest.raises(SchemaError, match="missing required columns.*report_month"):
        task.evaluate(paths[2], PredictionResult(prediction.scores.drop("report_month")))

    mismatched_scores = prediction.scores.with_columns(
        pl.when(pl.int_range(pl.len()) == 0).then(pl.col("epk_id") + 10_000_000).otherwise(pl.col("epk_id")).alias("epk_id")
    )
    ...
    restored_values = restored.predict(paths[2]).scores.select(UPLIFT_SCORE_COLUMNS).to_numpy()
    ...
    assert "target" not in production.scores.columns and "treatment" not in production.scores.columns


def test_combined_layout_returns_independent_branches_and_rejects_unknown_group(tmp_path) -> None:
    frames = []
    for seed in (11, 12, 13):
        frame = _split(96, seed).with_columns(
            pl.when(pl.int_range(pl.len()) % 4 < 2).then(pl.lit("a")).otherwise(pl.lit("b")).alias("group")
        )
    ...
    expected = task._predict_entry(task._read(unknown_path).with_row_index("__row_id"), global_model)
    global_only = task.predict(unknown_path, model_layout="global").scores.select(UPLIFT_SCORE_COLUMNS).to_numpy()
...
def test_group_scope_is_forbidden_without_group_column(tmp_path) -> None:
    with pytest.raises(ConfigError, match="model_layout='per_group'.*group_column=None"):
        UpliftTaskConfig(
            env_type="local",
            backend="boosting",
            engine="xgboost",
...
def test_estimated_propensity_is_fitted_saved_and_bounded(tmp_path) -> None:
    ...
    paths = [tmp_path / f"propensity-{name}.parquet" for name in ("train", "valid", "test")]
    for frame, path in zip((train, valid, test), paths, strict=True):
        frame.write_parquet(path)
    task = UpliftTask(
        UpliftTaskConfig(
    ...
    np.testing.assert_allclose(restored.predict(paths[2]).scores["score_x_propensity"].to_numpy(), propensity)

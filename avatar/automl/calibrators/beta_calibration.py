...
class BetaCalibrator:
    ...
    def fit(cls, scores: np.ndarray, target: np.ndarray) -> "BetaCalibrator":
        ...
        if values.ndim != 1 or values.size != targets.size or values.size < 2 or not np.isfinite(values).all():
        ...
        if set(np.unique(targets).tolist()) != {0, 1}:
            ...
            raise SchemaError(msg)
        model = BetaCalibration().fit(values.reshape(-1, 1), targets.astype(int)).calibrator_
        return cls(

...
def test_hive_partition_columns_above_and_below_split_root_are_preserved(tmp_path):
    ...
    pl.DataFrame({"epk_id": [1], "feature": [0.1]}).write_parquet(first_month / "part-00000.parquet")
    pl.DataFrame({"epk_id": [2], "feature": [0.2]}).write_parquet(first_month / "part-00001.parquet")
    pl.DataFrame({"epk_id": [3], "feature": [0.3]}).write_parquet(second_month / "part-00000.parquet")

    frame = ParquetSource.resolve(split_root).read().sort("epk_id")

    assert frame.columns == ["epk_id", "feature", "global_name", "split_type", "month_part"]
    ...
    assert source.unique_column_values("month_part") == (("2025-10-31", "2025-11-30"), False)


def test_unique_column_values_reads_only_requested_physical_column_and_detects_nulls(tmp_path):
    pl.DataFrame({"group": ["b", None], "unused": [1, 2]}).write_parquet(tmp_path / "first.parquet")
    pl.DataFrame({"group": ["a", "b"], "unused": [3, 4]}).write_parquet(tmp_path / "second.parquet")
...
def test_lazy_projection_preserves_hive_union_nulls_and_physical_precedence(tmp_path, monkeypatch):
    first = tmp_path / "group=a"
    second = tmp_path / "group=b"
    first.mkdir()
    second.mkdir()
    pl.DataFrame({"id": [2, 1], "x": [1.0, None], "unused": ["large", "payload"]}).write_parquet(first / "a.parquet")
    pl.DataFrame({"id": [3], "group": ["physical"], "unused": ["payload"]}).write_parquet(second / "b.parquet")
    source = ParquetSource.resolve(tmp_path)
    ...
    assert result.to_dict(as_series=False) == {"id": [2, 1, 3], "x": [1.0, None, None], "group": ["a", "a", "physical"]}

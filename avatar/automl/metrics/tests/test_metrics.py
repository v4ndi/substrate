...
def test_binary_metrics_are_score_ordered() -> None:
    ...
    assert binary_top_k_metrics(target, scores, ks=(50,)) == {"precision@50": 1.0, "recall@50": 1.0}


@pytest.mark.parametrize("name", ["roc_auc_ovr_macro", "accuracy", "f1_macro", "log_loss"])
def test_multiclass_objectives_and_evaluation(name: str) -> None:
    ...
    assert np.isfinite(multiclass_objective(name, target, probabilities, ("a", "b", "c")))
...
def test_registry_resolves_scalar_matrix_and_treatment_aware_inputs() -> None:
    ...
    assert resolve_metric("accuracy", "multiclass", "optimization").compute(multiclass) == 1.0
    ...
    assert np.isfinite(resolve_metric("qini_auc", "uplift", "optimization").compute(uplift))


def test_registry_owns_compatibility_directions_and_defaults() -> None:
    assert resolve_metric("log_loss", "multiclass", "optimization").optimization_direction == "minimize"
    assert resolve_metric("roc_auc", "response", "optimization").optimization_direction == "maximize"
    assert tuple(metric.name for metric in resolve_evaluation_metrics(None, "multiclass")) == (
        DEFAULT_EVALUATION_METRICS["multiclass"]
    )


@pytest.mark.parametrize(("task", "name"), _EVALUATION_ONLY_CASES)
def test_every_evaluation_only_metric_is_rejected_for_optimization(task: str, name: str) -> None:
...
def test_incompatible_evaluation_metric_is_rejected_for_every_task(task: str, name: str) -> None:

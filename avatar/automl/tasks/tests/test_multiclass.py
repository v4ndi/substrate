...
def test_multiclass_documentation_is_self_contained_and_base_is_task_neutral():
    ...
    for path in notebook_paths:
        ...
        if path.parent.name == "tests":
            ...
            assert code_cells
            assert all(cell["execution_count"] is not None and cell["outputs"] for cell in code_cells)
            assert not any(output.get("output_type") == "error" for cell in code_cells for output in cell["outputs"])
    ...
    common_source = "\n".join(
        ...
        for relative in ("avatar/automl/tasks/base.py", "avatar/automl/backends/boosting/base.py")
    ).casefold()
    for task_specific in ("positive_class", "roc_auc", "mean_squared_error", "beta_calibration"):
        assert task_specific not in common_source


def _write_splits(tmp_path, *, labels=(0, 1, 2), calibration=False, group=True, hidden=False):
    ...
    for split, split_months in months.items():
        ...
        for month_index, month in enumerate(split_months):
            ...
            values = {
                ...
                "segment": np.where(feature < 0.7, "low", np.where(feature > 1.3, "high", "middle")),
                "balance": feature,
                "treatment": np.arange(size) % 2,
                "report_month": [month] * size,
            }
            if group:
                values["group"] = np.where(np.arange(size) % 2, "channel_a", "channel_b")
            if hidden:
...
@pytest.mark.parametrize(
    ("engine", "params"),
    [("catboost", {"iterations": 12, "depth": 3}), ("xgboost", {"n_estimators": 12, "max_depth": 3})],
)
def test_multiclass_lifecycle_probability_contract_reports_and_artifact(tmp_path, engine, params):
    pytest.importorskip(engine)
    train, valid, test = _write_splits(tmp_path, labels=("class z", "class a", "class m"), hidden=True)
    task = MulticlassTask(_config(tmp_path, engine=engine, model_params=params, hidden_state_columns=["seq_hidden_state"]))
    ...
    assert class_wise.metrics_by_class_raw.columns == ["class_index", "class_label", "roc_auc", "n_positives"]
    assert set(aggregate_only.metrics_raw) == {"roc_auc_ovr_macro"}
    assert aggregate_only.metrics_by_class_raw is None
    assert sum(evaluation.metrics_raw[f"n_positives_class_{index}"] for index in range(3)) == prediction.scores.height
    assert all(0 <= evaluation.metrics_raw[f"precision@10_class_{index}"] <= 1 for index in range(3))
    ...
    assert evaluation.metrics_by_class_raw["class_label"].to_list() == list(prediction.class_order)
    assert evaluation.metrics_by_class_raw.schema["class_label"] == pl.String
    assert evaluation.metrics_by_class_raw["n_positives"].sum() == prediction.scores.height
    ...
    assert artifact == (tmp_path / "reports" / "artifacts" / "multiclass_model").resolve()
    ...
    assert manifest["target_schema"] == {"dtype": "String", "class_order": list(prediction.class_order)}
...
def test_supported_class_order_is_deterministic_and_dtype_is_preserved(tmp_path, labels):
    ...
    expected_dtype = "Int64" if isinstance(labels[0], int) else "Float64" if isinstance(labels[0], float) else "String"
    assert manifest["target_schema"]["dtype"] == expected_dtype
    assert [item["label"] for item in manifest["target_label_encoding"]] == sorted(labels)
...
def test_per_class_top_k_metrics_match_binary_fmlib_contract(tmp_path):
    ...
    metrics = multiclass_class_metrics(target, probabilities, task._class_order, grouped=False)
...
def test_unknown_validation_and_evaluation_labels_are_rejected(tmp_path):
    ...
    pl.read_parquet(valid).with_columns(
        pl.when(pl.arange(0, pl.len()) == 0).then(pl.lit(99)).otherwise(pl.col("target")).alias("target")
...
def test_group_model_part_with_incomplete_multiclass_target_has_named_error(tmp_path):
    ...
    train_frame = pl.read_parquet(train).with_columns(
        pl.when(pl.col("target") == 2).then(pl.lit("channel_b")).otherwise(pl.col("group")).alias("group")
    )
    train_frame.write_parquet(train)
    task = MulticlassTask(_config(tmp_path, model_layout="per_group", model_params={"iterations": 4}))
...
def test_multiclass_model_layouts_and_feature_semantics(tmp_path, scope, expected):
    train, valid, test = _write_splits(tmp_path)
    task = MulticlassTask(_config(tmp_path, model_layout=scope, model_params={"iterations": 4}))
...
def test_group_and_combined_layouts_reject_unknown_groups(tmp_path, monkeypatch):
    ...
    pl.read_parquet(test).with_columns(pl.lit("new_channel").alias("group")).write_parquet(unknown)
    group = MulticlassTask(_config(tmp_path, model_layout="per_group", model_params={"iterations": 4}))
    group.train(train, valid)
    with pytest.raises(SchemaError, match=r"Unknown group values.*'new_channel'.*90 rows"):
        group.predict(unknown)

    both = MulticlassTask(_config(tmp_path, model_layout="global_and_per_group", model_params={"iterations": 4}))
    both.train(train, valid)
    for item in both._models:
        values = np.array([0.8, 0.1, 0.1]) if item.layout == "global" else np.array([0.1, 0.8, 0.1])
        monkeypatch.setattr(
            item.backend,
            "predict_score",
    ...
    assert np.allclose(prediction.scores.select("score_0", "score_1", "score_2"), [0.8, 0.1, 0.1])
...
def test_model_layout_is_forbidden_without_group_column(tmp_path, scope):
    with pytest.raises(ConfigError, match=rf"model_layout={scope!r}.*group_column=None"):
        _config(tmp_path, group_column=None, model_layout=scope, model_params={"iterations": 4})
...
def test_without_group_evaluation_contains_month_only_rows(tmp_path):
    ...
    assert {
        f"{metric}_class_{index}"
        for index in range(3)
        for metric in ("roc_auc", "n_positives", "precision@5", "recall@5", "precision@50", "recall@50")
    } <= set(evaluation.metrics_by_group_raw.columns)


def test_grouped_slice_with_missing_class_has_null_auc_and_other_metrics(tmp_path, caplog):
    train, valid, test = _write_splits(tmp_path)
...
def test_prediction_result_class_order_mismatch_and_score_shape_are_rejected(tmp_path):
    ...
    missing = PredictionResult(prediction.scores.drop("score_2"), class_order=prediction.class_order)
...
@pytest.mark.parametrize(
    ("metric", "expected"),
    [("roc_auc_ovr_macro", "max"), ("accuracy", "max"), ("f1_macro", "max"), ("log_loss", "min")],
)
def test_multiclass_hyperopt_direction_no_refit_and_best_fitted_trial(tmp_path, monkeypatch, metric, expected):
...
def test_named_metric_artifact_load_is_inference_safe(tmp_path):
    ...
    task = MulticlassTask(
        _config(tmp_path, optimization_metric="accuracy", model_params={"iterations": 4})
    )
    task.train(train, valid)

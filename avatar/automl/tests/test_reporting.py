...
from avatar.automl.reporting import ConfusionData, CurveData, EvaluationData, export_evaluation
...
def _workbook_rows(path):
    ...
    with ZipFile(path) as archive:
        shared = ElementTree.fromstring(archive.read("xl/sharedStrings.xml"))  # noqa: S314 - workbook generated by this test
        strings = ["".join(item.itertext()) for item in shared.findall("s:si", namespace)]
        sheet = ElementTree.fromstring(archive.read("xl/worksheets/sheet1.xml"))  # noqa: S314 - workbook generated by this test
    rows = []
    for row in sheet.findall("s:sheetData/s:row", namespace):
        ...
        for cell in row.findall("s:c", namespace):
            ...
            if value is not None:
                cells[cell.attrib["r"].rstrip("0123456789")] = strings[int(value.text)] if cell.get("t") == "s" else value.text
        rows.append(cells)
    headers = rows[0]
    return [{headers[column]: value for column, value in row.items()} for row in rows[1:]]
...
def test_export_preserves_workbooks_figures_and_aliases(tmp_path, task_name, kind):
    metrics = {
        ...
        "multiclass": {"roc_auc_ovr_macro": 0.75, "accuracy": 0.6, "f1_macro": 0.55, "f1_weighted": 0.58},
    ...
    by_class = (
        pl.DataFrame({"class_index": [0], "class_label": ["a"], "roc_auc": [0.75]}) if task_name == "multiclass" else None
    )
    ...
    confusions = {"multiclass_confusion_matrix": ConfusionData(np.eye(3), ("a", "b", "c"))} if task_name == "multiclass" else {}
    ...
    assert getattr(result, f"excel_paths_{kind}") == getattr(result, f"figures_{kind}") == {}
    ...
    assert [row["feature"] for row in _workbook_rows(excel_paths[f"feature_importance_{kind}"])] == ["x", "y"]
    trend = {
        "binary": "roc_auc_by_date",
        "regression": "regression_metrics_by_date",
        "multiclass": "multiclass_metrics_by_date",
    }
    expected_figures = {f"{trend[task_name]}_{kind}"} if task_name in trend else {f"qini_curves_{kind}"}
    if task_name == "multiclass":
    ...
    for key, figure in figures.items():
        ...
        if key.removesuffix(f"_{kind}") in trend.values():
            ...
            np.testing.assert_allclose(axis.lines[0].get_ydata(), [next(iter(metrics.values()))])
    if task_name == "uplift":
        np.testing.assert_allclose(figures[f"qini_curves_{kind}"].axes[0].lines[0].get_ydata(), [0.0, 0.2])
    if task_name == "multiclass":
        np.testing.assert_allclose(
            figures[f"multiclass_confusion_matrix_{kind}"].axes[0].images[0].get_array(), np.eye(3)
        )

...
* :class:`~avatar.pipeline.tabular.supervised.SupervisedLearner` — binary
  classification, regression and multi-class, told apart by ``num_classes``
  and ``task_type``;
* :class:`~avatar.pipeline.tabular.uplift.SLearner` — the same model with the
  treatment flag fed in as a feature, scored on the difference between a
  treated and a control pass.

:mod:`~avatar.pipeline.tabular.interaction` holds the blocks that decide how

...
`random_state` (`avatar/automl/config/base.py:230-238`). Резолв становится
**backend-aware** для двух из них: у `search_space` от бэкенда зависят допустимые
ключи, валидация и дефолт, у `n_trials` — подставляемое значение (50 у бустинга,
**10** у tabnn, 7.2). Поля остаются те же, добавляется только развилка в резолве.
...
Произведение — **18 комбинаций**, `n_trials` по умолчанию для tabnn — **10**.
Значит из 18 точек случайно выбираются 10 уникальных (7.3): правило полного
перебора при этих дефолтах **не срабатывает**, оно включится, если пользователь
поднимет `n_trials` до 18 или сузит пространство. Это осознанный компромисс:
десять трейлов на одной A100 — это прогон на ночь, а не на неделю.

**`n_trials` по умолчанию зависит от бэкенда.** Сейчас конфиг подставляет 50 при
`hyperopt=True` независимо от семьи
(`avatar/automl/config/base.py:347-349`); для tabnn дефолт становится 10 — там,
где у бустинга трейл занимает минуты, у сети он занимает часы. Явно заданный
`n_trials` имеет приоритет и не ограничивается.

Фиксированные, **не ищутся**: `dropout_p=0.2`, `batch_size=4096`, `num_heads=8`,
`attn_dropout=0.15`, `out_head_hidden_dim=256`, `aggregation="mean"`,
`weight_decay=1e-2`, `max_epochs=30`, `patience=5`, `amp`.

`num_heads=8` делит оба значения `hidden_size` (32/8 = 4, 64/8 = 8), поэтому
конфликта размерностей в дефолтном пространстве нет; пользовательский
`search_space` обязан сохранять делимость.
...
**`n_trials` — это потолок, а не количество.** Если уникальных наборов меньше,
чем `n_trials` (маленькая сетка, дедуп), фактическое число трейлов меньше
запрошенного — это должно быть явно написано в логе и отчёте, иначе выглядит как
потерянные трейлы. На дефолтном пространстве tabnn обратное: 18 комбинаций при
`n_trials=10`, то есть выполняется ровно 10 случайно выбранных.
...
## 15. Что разошлось с планом при реализации

Дизайн выше — план. Реализация прошла его целиком (S0–S15); здесь записано
только то, где результат отличается от написанного, чтобы разница не
обнаружилась при чтении кода.

**Критерий 13 сужен.** «Параллельный препроцессинг воспроизводит
последовательный `dump()` побитово» верно для словаря категорий — счётчики
целые, а порядок `sorted` не зависит от обхода — и **неверно для числовой
статистики**: сложение чисел с плавающей точкой неассоциативно, поэтому другая
группировка формулы Чана округляется иначе. Тесты утверждают `rel=1e-12` против
последовательного фита и против `numpy`. При `num_workers=1` путь побитово тот
же, и это проверяется на всём `dump()`. Обещать большее означало бы тест со
спрятанным допуском.

**`avatar` не исчезает из процесса.** `fmlib/__init__.py` импортирует
`fmlib.losses`, поэтому torch оказывается в `sys.modules` при любом импорте
чего угодно из `fmlib`. Свойство «бустинговый путь не тянет torch» проверяется
статически: ни один модуль `fmlib/automl/**`, кроме `backends/tabnn/`, не
импортирует torch. Отсюда же словарь загрузчиков вместо словаря классов в
`_backend_class_for`.

**Пула процессов на `transform` при одном воркере нет.** Число выходных файлов
следует числу исходных, а не числу воркеров: `num_workers=1` пишет один файл.
Требование «выход обязан состоять из нескольких файлов» выполняется тем, что
источник обычно шардирован, а не принудительным дроблением.

**Каденция валидации считается на ранг.** Это нашлось на двухранговом прогоне:
каденция, посчитанная по всему корпусу, оказывалась длиннее эпохи, трейл
обучался, ни разу не валидировался и возвращался без метрики — неотличимо от
падения.

**Утечка дескрипторов в раннере.** Каждый датасет держит shm-дескриптор счётчика
эпох, а `Trainer` держит лоадеры через колбэки: `del model` оставлял два
дескриптора на трейл. Раннер теперь отпускает трейнер, лоадеры, оптимизатор,
планировщик, метрики и колбэки.

**`estimate_propensity=True` отвергается для tabnn** — propensity принадлежит
X-метаобучателю, которого здесь нет. В плане этого пункта не было.

**Ноутбук-пример не написан.** Вместо него — раздел TabNN в
`examples/automl/README.md` и запускаемый `examples/automl/benchmark_backends.py`,
который обучает обе семьи на одних данных и печатает оговорку про признаки. Пять
существующих ноутбуков написаны под бустинг и остались такими.

## 16. Что нашлось после плана

**`run_spec.json` хранил конфиг строкой.** `build_train_config` возвращает
`DictConfig`, а `DictConfig` — не `dict`: `dataclasses.asdict` проходит мимо
него, и `json.dumps(default=str)` записывал в спеку *repr* конфига. Раннер
в процессе этого не замечал — он держит объект спеки и никогда не читает файл,
— поэтому весь поиск проходил зелёным. Любой раннер, который спеку читает,
а не держит, падал: `torchrun` завершался кодом 1 на каждом ранге и не писал
`result.json`; путь Osiris вёл бы себя так же.

Тест многоранговости это прятал: он сам звал `OmegaConf.to_container` перед
сборкой `TrialSpec`, то есть проверял форму, которой продакшн не производит.
Теперь он передаёт `DictConfig` ровно так, как это делает `fit_model_part`, и
без фикса падает. Нормализация живёт в `TrialSpec.__post_init__`, а не на месте
вызова, чтобы гарантия держалась для любого будущего производителя спеки.

**Сходимость сверена численно, а не структурно.** Утверждение «обучение не
переписано, всё строится тем же слоем `instantiate`/`init_*`, что и
`python -m fmlib.train`» — довод про код, а не свидетельство про числа.
`tests/automl/backends/tabnn/test_train_parity.py` гоняет трейл через публичный
API AutoML, отдаёт оставшийся `run_spec.json` настоящей точке входа
`fmlib.train` как Hydra-конфиг и сравнивает чекпойнты побайтово. Правятся ровно
две вещи: куда писать и имена `mlflow`, из которых точка входа строит свой путь
для чекпойнтов; ни модель, ни данные, ни оптимизатор, ни расписание, ни сид не
трогаются.

`checkpoint.pt` несёт модель, состояние оптимизатора, планировщика, ГСЧ и
трейнера, поэтому равенство байтов означает те же шаги в том же порядке с
остановкой в том же месте, а не похожий скор. Наблюдается: оба сохраняются на
шаге 64, sha256 совпадают для `model.bin` и `checkpoint.pt`, `best_metric`
совпадает до последней цифры, и число, которое AutoML отдал поиску, — это то,
что записал цикл. Сравнение не вырожденное: тот же конфиг с `seed: 43` даёт
другой хеш и другой лучший шаг.

...
оставлены как заглушки с `DeprecationWarning` на один релиз: сорок восемь
конфигов и внешние кластерные конфиги называют этот путь. Для
`TabularClassification` заглушки **нет** намеренно — у неё поменялись имена
аргументов, так что заглушка сохраняла бы вызов, которого больше не
существует. Конфиги в репозитории обновлены; снаружи рецепт такой:

```
_target_: avatar.pipeline.tabular.SupervisedLearner
```

плюс подъём содержимого `tabular_model:` на уровень выше, `encoder:` →
`tabular_encoder:`, `extra_hidden_dim:` → `hidden_state_dim:`.

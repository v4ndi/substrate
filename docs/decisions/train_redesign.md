...
### 6.6 Обновление 2026-09-13: имя точки входа и удаление `cam_inference`

Пункт про заглушки из §6.5 закрыт в другую сторону, чем предполагалось. Вместо
того чтобы дождаться переезда внешних конфигов и убрать `avatar.inference`,
основным именем оставлено оно же: файл `avatar/infer.py` переименован обратно в
`avatar/inference.py`, заглушки нет. Довод простой — имя `avatar.inference`
стоит в запусках вне репозитория и в ноутбуках `experiments/`, а `avatar.infer`
не прожил дольше одного релиза и нигде, кроме этого репозитория, не встречался.
Переименование дешевле для всех, кроме самого репозитория.

`avatar/cam_inference.py` удалён. Его цикл отличался от `avatar/inference.py`
ровно одним: разворачиванием батча в матрицу (задача x канал коммуникации)
перед скорингом. Это разворачивание — не точка входа, а объект, который
`predict` уже умеет принимать параметром `prepare_batch`; оно переехало в
`avatar/data/campaign.py` и теперь инстанцируется из конфига. Точка входа в
инференс в репозитории одна.

...
Выбор между ними — вопрос того, где живут внешние эмбеддинги: до агрегации или после неё.
...
поля `task_gated_weights` и `aux` выхода, по которым видно, распределяет ли
гейт нагрузку или схлопнулся на одного эксперта

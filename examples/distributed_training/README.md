...
За три эпохи функция потерь падает примерно с 0.74 до 0.16, а `valid_roc_auc_score`
поднимается до ~0.67: цель — зашумлённая линейная функция признаков, так что
учиться есть чему, но не до единицы.

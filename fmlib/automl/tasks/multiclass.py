...
from fmlib.automl.config import MulticlassTaskConfig
...
class MulticlassTask(SupervisedTask[MulticlassBoostingBackend]):
    ...
    def __init__(
        self, config: MulticlassTaskConfig, *, _entity_path: Path | None = None
    ):
    ...
    def _validate_known_target(self, frame: pl.DataFrame, *, split: str) -> None:
        ...
        unknown = [
            value
            for value in target.unique().to_list()
            if self._label_index(value) is None
        ]
        if unknown:
    ...
    def _target(self, frame: pl.DataFrame) -> np.ndarray:
        ...
        encoded = [
            ...
            for value in frame[self._internal_config.target_column].to_list()
        ]
    ...
    def _fit_one(
        ...
        *,
        ...
        if np.unique(valid_codes).size < 2:
            ...
            raise SchemaError(msg)
        if (
            self.config.optimization_metric == "roc_auc_ovr_macro"
            and set(np.unique(valid_codes).tolist()) != expected
        ):
            missing = [
                self._class_order[index]
                for index in sorted(expected - set(np.unique(valid_codes).tolist()))
            ]
    ...
    def _probability_frame(
        self, base: pl.DataFrame, probabilities: np.ndarray
    ) -> pl.DataFrame:
        if probabilities.shape != (base.height, len(self._class_order or ())):
            ...
            raise SchemaError(msg)
        columns = [
            pl.Series(name, probabilities[:, index])
            for index, name in enumerate(self._score_columns(probabilities.shape[1]))
        ]
        ...
        *,
        ...
        for metric in selected:
            if metric.name in class_wise_names:
                output_name = (
                    "roc_auc" if metric.name == "class_wise_roc_auc" else metric.name
                )
                for index in range(probabilities.shape[1]):
                    binary_target = (target == index).astype(np.int8)
                    values[f"n_positives_class_{index}"] = int(binary_target.sum())
                    if (
                        metric.name == "class_wise_roc_auc"
                        and np.unique(binary_target).size != 2
                    ):
                        value = None
                        if grouped:
                            logger.warning(
                        ...
                        value = metric.compute(
                            MetricInput(binary_target, probabilities[:, index])
                        )
                    values[f"{output_name}_class_{index}"] = value
                continue
            if metric.name == "roc_auc_ovr_macro" and set(np.unique(target)) != set(
                ...
                if not grouped:
                    missing = [
                        class_order[index]
                        for index in sorted(
                            set(range(len(class_order))) - set(np.unique(target))
                        )
                    ]
                ...
                value = None
                logger.warning(
                    "Grouped multiclass ROC AUC is undefined and will be null because target classes are missing"
                )
            else:
                value = metric.compute(
                    MetricInput(target, probabilities, class_order=class_order)
                )
    ...
    def _evaluation_frames(
        ...
        if isinstance(scores, PredictionResult):
            if (
                scores.class_order is not None
                and tuple(scores.class_order) != self._class_order
            ):
        ...
        def joined(
            external: pl.DataFrame, *, warn_duplicates: bool = True
        ) -> pl.DataFrame:
    ...
    def _metrics_by_class_table(
        ...
        scopes = [
            scope
            for scope in ("global", "per_group")
            if any(name.startswith(f"{scope}_") for name in metrics)
        ]
        if scopes:
            return pl.concat(
                [
        ...
        class_metrics = [
            name for name in metric_names if name == "class_wise_roc_auc" or "@" in name
        ]
        ...
        labels = pl.Series(
            "class_label", list(self._class_order or ()), dtype=label_dtype
        )
        values: dict[str, Any] = {"class_index": range(class_count)}
        if "class_wise_roc_auc" in class_metrics:
            values["roc_auc"] = [
                metrics[f"roc_auc_class_{index}"] for index in range(class_count)
            ]
        values["n_positives"] = [
            metrics[f"n_positives_class_{index}"] for index in range(class_count)
        ]
        for name in class_metrics:
            ...
            values[output_name] = [
                metrics[f"{output_name}_class_{index}"] for index in range(class_count)
            ]
        return pl.DataFrame(values).insert_column(1, labels)

    def _confusion_data(
        self, evaluated: pl.DataFrame, *, suffix: str = ""
    ) -> dict[str, ConfusionData]:
            ...
            for key, branch in evaluated.partition_by(
                ...
                matrices.update(
                    self._confusion_data(
                        branch.drop("model_layout"), suffix=f"_{scope}"
                    )
                )
            return matrices
        ...
        matrix = confusion_matrix(
            target, predicted, labels=np.arange(len(labels)), normalize="true"
        )
        return {
            f"multiclass_confusion_matrix{suffix}": ConfusionData(
                matrix, labels, suffix
            )
        }
    ...
    def _copy_task_state(self, other: BaseTask) -> None:
        if (
            ...
            and self._class_order != other._class_order
        ):
    ...
    def _task_manifest(self) -> dict[str, Any]:
        return {
            ...
            "target_label_encoding": [
                ...
                for label, encoded in self._target_encoding
            ],
        }
    ...
    def _restore_task_manifest(self, manifest: Mapping[str, Any]) -> None:
        ...
        if (
            not isinstance(target_schema, Mapping)
            or len(target_schema.get("class_order", ())) < 3
        ):
        ...
        self._target_encoding = [
            (item["label"], int(item["encoded"])) for item in encoding
        ]

...
:class:`~fmlib.pipeline.tabular.uplift.SLearner` subclasses this one and adds
...
from fmlib.losses.classification import ClassificationLoss
from fmlib.nn.embedding import BaseTabularEmbedding
from fmlib.nn.tabular import BaseTabularEncoder
from fmlib.nn.utils import get_aggregation_layer
from fmlib.nn.utils.ffn import FeedForwardNetwork
from fmlib.outputs import TabularOutput
from fmlib.pipeline.base import BasePipeline
from fmlib.pipeline.tabular.interaction import (
...
class SupervisedLearner(BasePipeline):
    ...
    the two is :meth:`~fmlib.losses.ClassificationLoss.align`'s job, not this
    class's.
    ...
    Args:
        ...
        aggregation_config: Config for
            :func:`~fmlib.nn.utils.get_aggregation_layer`; defaults to mean
        ...
        loss: Replace the loss. Defaults to
            :class:`~fmlib.losses.ClassificationLoss` built from
            ``num_classes`` and ``task_type``.

    Returns:
        :class:`~fmlib.outputs.TabularOutput` — logits always, loss only when
        targets were given.
    """

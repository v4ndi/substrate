"""Training entrypoint.
    ...
    torchrun --standalone --nproc_per_node=<gpus> -m fmlib.train \
        --config-dir=configs --config-name=<name>

Single-process runs need no launcher: ``python -m fmlib.train ...`` sees no
...
from fmlib.train.factory import build_callbacks, resolve_performance_config
...
from fmlib.utils.logging import print_meta, print_train_info
from fmlib.utils.performance_metrics import configure_mlflow_system_metrics
...
def main(config: DictConfig) -> None:
    """Build the run from config and hand it to :class:`~fmlib.train.loop.Trainer`.

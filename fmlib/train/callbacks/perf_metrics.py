...
Ports the performance block that used to be inlined in ``fmlib/train.py``.
All reductions use the scalar collectives in
:mod:`fmlib.utils.performance_metrics`, which are already plain
``torch.distributed``.
"""
...
from fmlib.utils.performance_metrics import (

...
@pytest.mark.parametrize(
    ("engine", "params"),
    [
        ("catboost", {"iterations": 16, "depth": 3}),
        ("xgboost", {"n_estimators": 16, "max_depth": 3}),
    ],
)
def test_regression_lifecycle_metrics_reports_and_artifact(tmp_path, engine, params):
    ...
    assert (
        task.path / "evaluations" / dataset_key / "regression_metrics_by_date_raw.png"
    ).exists()
    ...
    assert (
        artifact == (tmp_path / "reports" / "artifacts" / "regression_model").resolve()
    )
    assert np.allclose(
        prediction.scores["score"], restored.predict(test).scores["score"]
    )
...
def test_regression_target_preserves_numeric_dtype():
    task = RegressionTask(_config(None))
    target = task._target(
        pl.DataFrame({"target": pl.Series([1.0, 2.0], dtype=pl.Float32)})
    )
...
def test_regression_model_layouts_and_global_grouped_metrics(
    tmp_path, model_layout, expected
):
...
def test_group_and_combined_layouts_reject_unknown_groups(tmp_path, monkeypatch):
    ...
    for item in both._models:
        ...
        monkeypatch.setattr(
            item.backend,
            "predict_score",
            lambda frame, schema, value=value: np.full(frame.height, value),
        )
    with pytest.raises(SchemaError, match=r"Unknown group values.*'new'.*50 rows"):
...
def test_regression_search_space_float_step_and_log_validation(tmp_path):
    task = RegressionTask(
        _config(
            ...
            hyperopt=True,
            search_space={
                "eta": {"type": "float", "low": 0.1, "high": 0.3, "step": 0.1}
            },
        )
    )
    trial = _Trial()
...
def test_regression_uses_packaged_default_search_space(tmp_path, monkeypatch):
    ...
    task = RegressionTask(
        _config(tmp_path, model_params={"iterations": 7}, search_space=None)
    )
...
def test_regression_custom_aliases_and_recursive_hive_shards(tmp_path):
    ...
    for root, month in (
        (train_root, "2025-10-01"),
        (valid_root, "2025-11-01"),
        (test_root, "2025-12-01"),
    ):

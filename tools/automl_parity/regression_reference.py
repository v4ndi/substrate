...
def run_reference(case) -> dict:
    ...
    config = OmegaConf.to_container(
        OmegaConf.load(case.autocampaign_config_path), resolve=True
    )
    ...
    with tempfile.TemporaryDirectory(
        prefix="autocampaign_regression_", dir=case.output_dir
    ) as temporary:
        ...
        train_seconds = sum(
            _run_entrypoint(
                root / "trainer_booster.py", item, cwd=root, environment=environment
            )
        ...
        evaluate_seconds = _run_entrypoint(
            root / "evaluater_booster.py", runtime, cwd=root, environment=environment
        )
    ...
    for path in params_paths:
        ...
        selected_params[name] = ast.literal_eval(
    ...
    if calibrated is not None:
        _normalized_scores(calibrated, case, "prediction_cal").write_parquet(
    ...
    result = {
        ...
        "calibrated_test_metrics": (
            _metrics(
                calibrated[case.target_column].to_numpy(),
                calibrated["prediction_cal"].to_numpy(),
            )
            if calibrated is not None
            else None
        ),

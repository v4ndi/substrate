...
def test_reference_client_artifacts_are_normalized_from_real_metalearner_labels(
    tmp_path,
) -> None:
    labels = [
        "<class 'causalml.inference.meta.slearner.BaseSClassifier'>",
        "BaseTLearner",
        "BaseXLearner",
    ]
...
def test_reference_calibrated_client_artifacts_use_calibrated_probability_columns(
    tmp_path,
) -> None:
    ...
    assert scores.select("score_s", "score_t", "score_x").row(0) == pytest.approx((
        0.4,
        0.41,
        0.42,
    ))
...
def test_production_package_has_no_causalml_import() -> None:
    ...
    production_files = [
        path
        for path in automl_root.rglob("*.py")
        if "_parity" not in path.parts and "tests" not in path.parts
    ]
    assert all(
        "import causalml" not in path.read_text(encoding="utf-8")
        for path in production_files
    )


def test_native_date_resolver_can_resolve_reference_config(
